...
class DiagnosticPackageModule:
    ...
    def collect(self, host: str) -> CommandResult:
        script = """
...
"""
        ...
        try:
            ...
            return CommandResult.failure(
                host,
                "diagnostic_package",
                f"Coleta concluída, mas não foi possível salvar o pacote: {exc}",
            )

        result.command = "diagnostic_package"
        result.stdout = f"Pacote de diagnóstico salvo em {target}"
        result.data = {"path": str(target), "payload": result.data}
        result.metadata["artifact_path"] = str(target)
        return result
